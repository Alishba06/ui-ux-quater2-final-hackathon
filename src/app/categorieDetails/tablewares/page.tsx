...
async function fetchAllTablewares(): Promise<TablewareProduct[]> {
  const query = `*[_type == "tablewares"] {
    _id,
    name,
    price,
    image {
  ...
  const results = await client.fetch<TablewareProduct[]>(query);
  return results;
}
...
export default async function TablewarePage({
  ...
  const allTablewares = await fetchAllTablewares();
  const selectedProduct = searchParams.id
    ? await fetchTablewareProduct(searchParams.id)
    : null;
  ...
  return (
    <div className="container mx-auto px-4 mt-10">
      ...
      {/* Display All Tableware Products */}
      <div>
        <h2 className="text-2xl font-bold mb-6">All Tableware Products</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {allTablewares.map((product) => (
            <div key={product._id} className="w-full">
              <Link
                href={`/categorieDetails/tablewares/${product._id}`}
                passHref
              >
                <img
              ...
              <p className="text-[#2A254B] font-clash text-lg font-normal leading-[140%] mt-3 mb-2">
                <Link
                  href={`/categorieDetails/tablewares?id=${product._id}`}
                  passHref
                >
                  {product.name}
                </Link>

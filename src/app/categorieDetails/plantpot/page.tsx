...
async function fetchAllPlants(): Promise<PlantProduct[]> {
  ...
  const results = await client.fetch<PlantProduct[]>(query);
  return results;
}
...
export default async function PlantPage({
  ...
  const selectedProduct = searchParams.id
    ? await fetchPlantProduct(searchParams.id)
    : null;
  ...
  return (
    <div className="container mx-auto px-4 mt-10">
      ...
      {/* Display All Plant Products */}
      <div>
        <h2 className="text-2xl font-bold mb-6">All Plant Products</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {allPlants.map((product) => (
            <div key={product._id} className="w-full">
              <Link
                href={`/categorieDetails/plantpot?id=${product._id}`}
                passHref
              >
                <img
